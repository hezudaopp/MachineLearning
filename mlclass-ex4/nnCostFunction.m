...
for i = 1:m
	a1 = [1 X(i, :)];	% a1 1 x 401
	z2 = a1 * Theta1';  % Theta1 25 x 401, z2 1 x 26
	a2 = [1 sigmoid(z2)]; % a2 1 x 26
	z3 = a2 * Theta2'; % Theta2 10 x 26, z3 1 x 10
	a3 = sigmoid(z3); % a3 1 x 10
	K = size(a3, 2);
	tmp = zeros(K, 1);
	tmp(y(i,:),1) = 1;
	J = J - (log(a3)) * (tmp) - (log(1 - a3)) * (1 - tmp);
	delta3 = a3' - tmp; % delta3 10 x 1
	delta2 = Theta2' * delta3 .* sigmoidGradient([1; z2']); % 26 x 10 * 10 x 1 .* 26 x 1
	Delta2 = Delta2 + delta3 * a2; % 10 x 26 + 10 x 1 * 1 x 26
	Delta1 = Delta1 + delta2(2:end) * a1; % 25 x 401 + 25 x 1 * 1 x 401 
end
J = J/m;
